...
class EarthquakeMap {
    ...
    initMapBackground() {
        try {
            ...
            console.log('地图背景初始化完成');
        } catch (error) {
            console.error('初始化地图背景时发生错误:', error);
        }
    }
    ...
    // 批量更新标记 - 优化：只添加新标记和移除不再需要的标记
    updateMarkers(earthquakes = null) {
        // 如果没有提供地震数据，只更新现有标记的位置和大小
        if (!earthquakes) {
            this.markers.forEach(markerInfo => {
                // 更新位置
        ...
        // 优化：基于地震数据更新标记
        const currentMarkerIds = new Set(this.markers.map(markerInfo => markerInfo.id));
        const newQuakeIds = new Set(earthquakes.map(quake => quake.id));
        
        // 1. 移除不再需要的标记
        const markersToRemove = this.markers.filter(markerInfo => !newQuakeIds.has(markerInfo.id));
        markersToRemove.forEach(markerInfo => {
            if (markerInfo.element && markerInfo.element.parentNode) {
                // 添加淡出动画
                markerInfo.element.style.opacity = '0';
                markerInfo.element.style.transition = 'opacity 0.3s ease-out';
                setTimeout(() => {
                    if (markerInfo.element && markerInfo.element.parentNode) {
                        markerInfo.element.parentNode.removeChild(markerInfo.element);
                    }
        ...
        // 从标记数组中移除
        this.markers = this.markers.filter(markerInfo => newQuakeIds.has(markerInfo.id));
        
        // 2. 添加新标记
        const newQuakes = earthquakes.filter(quake => !currentMarkerIds.has(quake.id));
        newQuakes.forEach(quake => {
            this.addMarker(quake);
        });
        
        // 3. 更新现有标记的地震数据
        this.markers.forEach(markerInfo => {
            const quake = earthquakes.find(q => q.id === markerInfo.id);
            if (quake && quake !== markerInfo.quake) {
                this.updateMarker(markerInfo, quake);
            }
        });
    }
