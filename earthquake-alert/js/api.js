...
class EarthquakeAPI {
    ...
    async fetchJMAQuakeData() {
        try {
            ...
            const parsedData = this.parseJMAQuakeData(rawData);
            
            // 实现增量更新，只返回新的地震数据
            if (this.config.data.enableIncrementalUpdate && this.cache.lastQuakeId) {
                const newQuakes = parsedData.filter(quake => {
            ...
            if (parsedData.length > 0) {
                this.cache.lastQuakeId = parsedData[0].id;
            }
    ...
    parseJMAQuakeData(rawData) {
        ...
        for (const item of rawData) {
            try {
                // 获取地震详情文件URL
                const detailUrl = `${this.config.api.jma_quake.detailUrl}${item}`;
                
                // 由于我们无法在同步方法中进行异步请求，
                // 这里我们只解析列表数据，后续可以通过单独的请求获取详细信息
                // 对于实时速报，列表数据已经包含了基本信息
                ...
                const quake = {
                    id: id,
                    time: new Date().toISOString(), // 后续会从详情中更新
                    location: '解析中...',
                    locationZh: '解析中...',
                    locationOriginal: '解析中...', // 地震国原语言
                    magnitude: 0,
                    depth: 0,
                    lat: 35.0,
                    lng: 135.0,
                ...
                console.error('解析JMA地震数据失败:', parseError);
            }
        }
        
        return earthquakes.filter(item => item.id); // 过滤掉无效的地震数据
    }
...
// 创建API实例
const earthquakeAPI = new EarthquakeAPI(config);
